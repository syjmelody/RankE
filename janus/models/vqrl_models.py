    ...
    def forward(self, images, texts):
        ...
        with torch.no_grad():
            if self.weights.get("image_reward", 0.0) != 0 and self.ir_model is not None:
                pil_imgs = [to_pil_image(img.clamp(0, 1)) for img in images]
            ...
            if self.weights.get("hpsv2", 0.0) != 0 and HAS_HPSV2:
                pil_imgs = [to_pil_image(img.clamp(0, 1)) for img in images]
                curr_texts = texts if len(texts) == batch_size else [t for t in texts for _ in range(batch_size // len(texts))]
                try:
                    hps_values = hpsv2.score(pil_imgs, curr_texts)
                    rewards_breakdown["hpsv2"] = torch.tensor(hps_values, device=device) * 100.0
                except:
...
class HybridVQLoss(nn.Module):
    ...
    def calculate_adaptive_weight(self, nll_loss, g_loss, last_layer):
        ...
        if last_layer is not None:
            # [Fix 1] Ensure the input loss requires gradients; otherwise exit early
            if not nll_loss.requires_grad or not g_loss.requires_grad:
                return torch.tensor(self.disc_weight, device=nll_loss.device)

            # Get the reconstruction-loss gradient with respect to the last layer
            nll_grads = torch.autograd.grad(nll_loss, last_layer, retain_graph=True)[0]
            
            # [Fix 2] Core fix: set allow_unused=True
            g_grads_tuple = torch.autograd.grad(g_loss, last_layer, retain_graph=True, allow_unused=True)
            g_grads = g_grads_tuple[0]
            
            # [Fix 3] If the discriminator path is disconnected (g_grads is None), return the base weight
            if g_grads is None:
                return torch.tensor(self.disc_weight, device=nll_loss.device)
            ...
            # Clamp the value to avoid numerical instability
            d_weight = torch.clamp(d_weight, 0.0, 1e4).detach()
            
            # Multiply by the base weight
