#!/bin/bash
#SBATCH --job-name=ranke_janus_train_clip
...
TRAIN_MODE="both"
EXP_PREFIX="ranke_${TRAIN_MODE}_blip3o" 
SCALING_SIZE="15k"
...
BATCH_SIZE_PER_GPU=2           # Batch size per GPU (Janus often uses 1, LlamaGen often uses 4)
GRAD_ACCUM=2                   # Gradient accumulation steps

# --- 5. Hybrid training weights (RL and decoder losses) ---
KL_COEF=0.05                   # GRPO KL penalty
LAMBDA_DEC_GAN=1.0             # GAN loss weight
LAMBDA_DEC_REWARD=0.05         # Direct reward-backprop weight
...
# --- Decoder consistency scheduling (aligned with LlamaGen) ---
...
CONSISTENCY_END_VALUE=5.0         # Final lambda_decoder_consistency value
...
# --- Four reward weights (kept consistent with LlamaGen) ---
R_CLIP=1.0
R_AESTHETIC=0.0
R_IMAGE_REWARD=0.0
R_HPSV2=0.0
...
# Build dataset paths
SCALING_DATA_ROOT="${SCALING_BLIP3O_ROOT}"
TARGET_TAR_PATH="${SCALING_DATA_ROOT}/train_subset_${SCALING_SIZE}.tar"
[ ! -f "$TARGET_TAR_PATH" ] && echo "Error: Dataset tar not found" && exit 1
...
# ==============================================================================
# [4] Assemble arguments and launch (LlamaGen-aligned)
# ==============================================================================
...
echo "========================================================================"
echo ">>> Launching Unified Hybrid RL"
