...
# --- Imports ---
try:
    from autoregressive.post_train.trainer import HybridRLTrainer, HybridTrainConfig
except ImportError:
    try:
        from autoregressive.post_train.trainer import HybridRLTrainer, HybridTrainConfig
    except ImportError:
        print("Error: Could not import HybridRLTrainer.")
        sys.exit(1)

try:
    from dataset.sft_dataset import build_dataloader
except ImportError:
    print("Error: Could not import 'build_dataloader' from 'dataset/sft_dataset.py'.")
    sys.exit(1)
...
try:
    ...
    print("Error: LlamaGen modules not found.")
    sys.exit(1)
...
def load_models(args, device):
    ...
    # 1. VQ Model
    print(f">>> [Loader] Loading VQ ({args.vq_model})")
    ...
    if isinstance(vq_ckpt, dict) and 'model' in vq_ckpt:
        vq_state = vq_ckpt['model']
    else:
        vq_state = vq_ckpt
    vq_model.load_state_dict(vq_state)
    vq_model.to(device).eval()
    
    ema_vq_model = VQ_models[args.vq_model](codebook_size=args.codebook_size, codebook_embed_dim=args.codebook_embed_dim)
    ema_vq_model.load_state_dict(vq_state)
    ema_vq_model.to(device).eval().requires_grad_(False)
    
    # 2. T5 Model
    print(f">>> [Loader] Loading T5 ({args.t5_model_type})")
    t5_model = T5Embedder(device=device, local_cache=True, cache_dir=args.t5_path, dir_or_name=args.t5_model_type,
                          torch_dtype=torch.bfloat16 if args.mixed_precision == 'bf16' else torch.float32, 
                          model_max_length=args.t5_max_len)

    # 3. GPT Model
    print(f">>> [Loader] Loading GPT from: {args.gpt_ckpt}")
    ...
    if args.kl_coef > 0 and args.use_fixed_ref_model:
        print(">>> [Loader] Initializing Fixed Reference Model")
        ref_gpt_model = GPT_models[args.gpt_model](block_size=latent_size**2, cls_token_num=args.cls_token_num, model_type='t2i')
        ref_gpt_model.load_state_dict(new_state_dict, strict=False)
        ref_gpt_model.to(device).eval().requires_grad_(False)

    print(">>> [Loader] Initializing EMA GPT Model")
    ema_gpt_model = GPT_models[args.gpt_model](block_size=latent_size**2, cls_token_num=args.cls_token_num, model_type='t2i')
    ema_gpt_model.load_state_dict(new_state_dict, strict=False)
    ema_gpt_model.to(device).eval().requires_grad_(False)

    # 5. Reward Manager
    print(">>> [Loader] Initializing Reward Manager")
    ...
    return gpt_model, vq_model, t5_model, reward_model, ref_gpt_model, ema_gpt_model, ema_vq_model

def main():
    parser = argparse.ArgumentParser(description="Hybrid RL training entry point")
    ...
    parser.add_argument("--uniform-advantages", action="store_true", default=False,
                        help="Ablation: set all advantages to 1.0 (disable ranking signal)")
    ...
    parser.add_argument("--wandb-project", type=str, default="ranke_llamagen")
    ...
    # ==================== Models & Trainer ====================
    (gpt_model, vq_model, t5_model, reward_model, 
     ref_gpt_model, ema_gpt_model, ema_vq_model) = load_models(args, accelerator.device)
    ...
    # [Critical fix]
    # Never call accelerator.prepare on a DataLoader that yields string lists.
    # dataloader = accelerator.prepare(dataloader)  # intentionally removed
    ...
    accelerator.print(f"Start v3 Alternating Training | Mode: {args.train_mode.upper()} | Total Steps: {total_steps}")
            
    for epoch in range(start_epoch, args.max_epochs):
        for batch in dataloader:
            # [Key step] Move tensors to the device manually
            # batch = (pixel_values, text_list)
            ...
            # ----------------------------------------------
                ...
                # # Counters
                # logs["counter/gpt_opt_step"] = trainer.counter_gpt_update
                # logs["counter/decoder_opt_step"] = trainer.counter_vq_update
                # logs["counter/disc_opt_step"] = trainer.counter_disc_update
                # logs["counter/ema_gpt_update"] = trainer.counter_ema_gpt_update
                # logs["counter/ema_vq_update"] = trainer.counter_ema_vq_update
            ...
            if accelerator.is_main_process:
                if global_step % args.log_interval == 0:
                    postfix_logs = {"rew": f"{logs.get('reward/clip_mean', 0):.2f}"}
                    ...
                    if 'reward/total' in logs:
                        postfix_logs["rew"] = f"{logs['reward/total']:.2f}"
            ...
            if global_step % 100 == 0:
                ...
                accelerator.print(f"Sync GPT & Decoder Update: {args.sync_gpt_decoder_update}")
            ...
            if is_save_step or is_sample_step:
                ...
                # A. Save Checkpoint (Updated with EMA support)
                if is_save_step:
                ...
                if is_sample_step:
                    ...
                    if len(local_prompts) > 0:
                        ...
                        # Do NOT barrier here: ranks with no prompts would wait at the outer barrier
                        # and cause collective mismatch (BARRIER vs GATHER). Sync only once below.
