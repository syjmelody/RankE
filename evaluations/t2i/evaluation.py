...
#     parser.add_argument("--fake_dir", required=True, default="/path/to/fake_images", help="location of fake images for evaluation")
#     parser.add_argument("--ref_dir",  required=True, default="/path/to/reference_images", help="location of the reference images for evaluation")
...
def compute_clip_score(
    dataset: DataLoader, clip_model="ViT-B/32", device="cuda", how_many=5000):
    print("Computing CLIP score (Optimized GPU Version)")
    import clip as openai_clip 
    import torchvision.transforms as T
    
    if clip_model == "ViT-B/32":
        clip, _ = openai_clip.load("ViT-B/32", device=device) # the original preprocessor is not needed here
        clip = clip.eval()
    elif clip_model == "ViT-G/14":
        import open_clip
        clip, _, _ = open_clip.create_model_and_transforms("ViT-g-14", pretrained="laion2b_s12b_b42k")
        clip = clip.to(device).eval().float()
    else:
        raise NotImplementedError

    # CLIP normalization statistics, applied directly on GPU
    ...
    count = 0
    for imgs, txts in tqdm(dataset):
        # 1. Move the tensor directly to GPU
        imgs = imgs.to(device)
        
        # 2. Convert dataset outputs from [-1, 1] back to [0, 1]
        imgs = (imgs + 1.0) / 2.0
        
        # 3. Resize on GPU to CLIP's expected 224x224 resolution
        imgs = F.interpolate(imgs, size=(224, 224), mode='bicubic', align_corners=False)
        
        # 4. Normalize directly on GPU
        imgs = clip_normalize(imgs)

        # ---- Keep the upstream text-processing logic unchanged ----
        tokens = openai_clip.tokenize(txts, truncate=True).to(device)
        prepend_text = "A photo depicts "
        prepend_text_token = openai_clip.tokenize(prepend_text)[:, 1:4].to(device)
        prepend_text_tokens = prepend_text_token.expand(tokens.shape[0], -1)
